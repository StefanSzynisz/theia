...
%           - avBCv  - average BC flag (avBC=1, av. BCs included)
%           - VTKout - VTK output flag 
%           - nelblo - no. elements/block (for vectorisation)
%           - itMax  - maximum number of iterations in the algorithm 
%           - tol    - tolerance on the algorithm
%           - SfN    - strain data file name
%--------------------------------------------------------------------------
% See also:
% 
...
BCt    = [2  -2  2  -2];                                                    % boundary condition flags
BCu    = [0 0;                                                              % boundary condition displacements (x,y) for each edge
          0 0; 
          0 0; 
          0 0];                                                             
BCp    = [0e6 -100e6 0e6 -100e6];                                           % boundary condition pressures (normal) for each edge
avBC   = 0;                                                                 % average BC flag (avBC=1, av. BCs included)
...
%% Strain data
StrainFileName = 'Strain.xlsx';                                             % strain data xlsx file

%% Other settings/output options
VTKout = 1;                                                                 % VTK output flag (= 1, output files generated)
...
hx = lx/nelsx;                                                              % element spacing (x direction)
hy = ly/nelsy;                                                              % element spacing (y direction)
...
mesh.avBC   = avBC;                                                         % average BC flag (avBC=1, av. BCs included)
mesh.VTKout = VTKout;                                                       % VTK output flag 
mesh.nelblo = nelblo;                                                       % no. elements/block (for vectorisation)
mesh.itMax  = itMax;                                                        % maximum number of iterations
mesh.tol    = tol;                                                          % convergence tolerance
mesh.SfN    = StrainFileName;                                               % name of the xls file with the strain data
